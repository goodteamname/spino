...
def decomp_tab(ts):
    ...
    def update_poly(attrname, old, new):
        if poly_select.value == 1:
            ...
            deseason_data = remove_seasonality(detrended_data, np.pi*2)
            combine2 = [ts, deseason_data]
            combined_df2 = pd.concat(combine2)
            newsource2 = ColumnDataSource(data=combined_df2)
            source2.data.update(newsource2)
    ...
    def update_season(attrname, old, new):
        if season_select.value == 0:
            ...
            detrended_data = remove_seasonality(ts, season_select.value)
            combine = [ts, detrended_data]
            combined_df = pd.concat(combine)
            newsource2 = ColumnDataSource(data=combined_df)
            source2.data.update(newsource.data)
    ...
    season_select = Slider(start=0, end=ts.time, step=1, value=0, title='Season period')
    ...
    combine2 = [ts, deseason_data]
    combined_df2 = pd.concat(combine2)
    source2 = ColumnDataSource(data=combined_df2)
    plot2 = make_seasonplot(source2)

    # Set up layouts and add to document
    # Put controls in a single element.
    controls = WidgetBox(text, poly_select, text2)
